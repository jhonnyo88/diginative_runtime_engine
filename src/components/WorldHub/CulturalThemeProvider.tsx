...
export const CulturalThemeProvider: React.FC<CulturalThemeProviderProps> = ({ children }) => {
  ...
  // Get current theme
  const currentTheme = culturalThemes[culturalContext];

  // Create Chakra UI theme based on cultural theme
  const chakraTheme = useMemo(() => {
    ...
    return extendTheme({
      ...
      colors: {
        primary: {
          50: currentTheme.colors.secondary,
          100: currentTheme.colors.accent + '20',
          200: currentTheme.colors.accent + '40',
          300: currentTheme.colors.accent + '60',
          400: currentTheme.colors.accent + '80',
          500: currentTheme.colors.primary,
          600: currentTheme.colors.primary,
          700: currentTheme.colors.primary,
          800: currentTheme.colors.primary,
          900: currentTheme.colors.primary,
        },
        accent: {
          50: currentTheme.colors.accent + '10',
          100: currentTheme.colors.accent + '20',
          200: currentTheme.colors.accent + '40',
          300: currentTheme.colors.accent + '60',
          400: currentTheme.colors.accent + '80',
          500: currentTheme.colors.accent,
          600: currentTheme.colors.accent,
          700: currentTheme.colors.accent,
          800: currentTheme.colors.accent,
          900: currentTheme.colors.accent,
        }
      },
      fonts: {
      ...
      letterSpacings: currentTheme.typography.letterSpacing,
      styles: {
        global: {
          body: {
        ...
        Button: {
          ...
          variants: {
            solid: {
              ...
              color: 'white',
              _hover: {
                bg: currentTheme.colors.primary + 'DD',
      ...
      space: {
        cardPadding: currentTheme.spacing.cardPadding,
        sectionSpacing: currentTheme.spacing.sectionSpacing,
      }
    });
  }, [currentTheme]);
  ...
  const switchTheme = (themeName: string) => {
    ...
    const personaName = themePersonaMap[themeName];
    if (personaName) {
      // Switch persona which will trigger theme change
      const persona = { name: personaName }; // Simplified persona object
...
export const CulturalThemeSelector: React.FC = () => {
  const { currentTheme, switchTheme, availableThemes, culturalContext } = useCulturalTheme();

  return (
    <HStack spacing={2} flexWrap="wrap">
      {availableThemes.map((themeName) => {
        const theme = culturalThemes[themeName];
        const isActive = culturalContext === themeName;
        ...
        return (
          <Button
            ...
            _hover={{
              bg: isActive ? theme.colors.primary + 'DD' : theme.colors.primary + '10',
              transform: 'translateY(-1px)',
            }}
...
// Export cultural themes for direct usage
export { culturalThemes, type CulturalTheme };