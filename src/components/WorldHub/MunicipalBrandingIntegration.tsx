...
export const MunicipalBrandingIntegration: React.FC<MunicipalBrandingIntegrationProps> = ({
  ...
  const relevantConfigurations = useMemo(() => {
    const contextCountryMap: Record<string, string> = {
      swedish: 'Sweden',
      german: 'Germany',
      french: 'France',
      dutch: 'Netherlands'
    ...
    const targetCountry = contextCountryMap[culturalContext] || 'Sweden';
    return municipalBrandingConfigurations.filter(config => 
      config.country === targetCountry
    );
  }, [culturalContext]);

  const activeConfiguration = municipalBrandingConfigurations.find(
    config => config.id === selectedConfiguration
  ) || municipalBrandingConfigurations[0];

  const handleConfigurationSelect = (configurationId: string) => {
  ...
  return (
    <Container maxW="7xl" py={8}>
      <VStack spacing={8} align="stretch">
        ...
        >
          <VStack spacing={4} align="stretch">
            <HStack justify="space-between" align="center">
              <HStack spacing={3}>
                <Icon as={FiShield} w={8} h={8} color={activeConfiguration.visualIdentity.colorScheme.primary} />
                <VStack align="start" spacing={0}>
                  <Text fontSize="2xl" fontWeight="800" color={activeConfiguration.visualIdentity.colorScheme.primary}>
                    Municipal Professional Branding Integration
                  </Text>
                  <Text fontSize="md" color="gray.600">
                    Seamless integration with official municipal identity and professional development
                  </Text>
                </VStack>
              </HStack>
            ...
            <Card bg={activeConfiguration.visualIdentity.colorScheme.primary + '05'} borderColor={activeConfiguration.visualIdentity.colorScheme.primary + '30'}>
              <CardBody p={4}>
                <HStack justify="space-between">
                  <HStack spacing={4}>
                    <Box
                      w={10}
                      h={10}
                      bg={activeConfiguration.visualIdentity.colorScheme.primary}
                      borderRadius="md"
                      display="flex"
                      alignItems="center"
                      justifyContent="center"
                    >
                      <Text color="white" fontWeight="700" fontSize="sm">
                        {activeConfiguration.municipalityName.substring(0, 2).toUpperCase()}
                      </Text>
                    </Box>
                    <VStack align="start" spacing={0}>
                      <Text fontSize="lg" fontWeight="700" color={activeConfiguration.visualIdentity.colorScheme.primary}>
                        Active: {activeConfiguration.displayName}
                      </Text>
                      <Text fontSize="sm" color="gray.600">
                        {activeConfiguration.certificationLevels.current} • {activeConfiguration.population.toLocaleString()} citizens
                      </Text>
                    </VStack>
                  </HStack>
                  ...
                  <VStack align="end" spacing={1}>
                    <Badge colorScheme="green" variant="solid" p={2} borderRadius="lg">
                      {activeConfiguration.stakeholderRecognition.citizenAwareness}% Citizen Awareness
                    </Badge>
                    <Text fontSize="xs" color="gray.600">
        ...
        {/* Municipal Configuration Selection */}
        <Box>
          <Text fontSize="lg" fontWeight="600" color="gray.700" mb={4}>
            Available Municipal Configurations:
          </Text>
          
          <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)", lg: "repeat(3, 1fr)" }} gap={6}>
            {relevantConfigurations.map((configuration) => (
              <MunicipalBrandingCard
                key={configuration.id}
                configuration={configuration}
                isActive={configuration.id === selectedConfiguration}
                onClick={() => handleConfigurationSelect(configuration.id)}
              />
            ))}
          </Grid>
        </Box>
        ...
        {showDetailedView && (
          <MunicipalBrandingDetails configuration={activeConfiguration} />
        )}
